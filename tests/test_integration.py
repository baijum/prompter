...
'''
    ...
    def test_runner_with_real_commands(self, mock_subprocess, complete_config_file, temp_dir):
        ...
        # Verify subprocess was called for both Claude commands and verification
        assert mock_subprocess.call_count == 4  # 2 tasks × 2 commands each
    ...
    @patch('subprocess.run')
    def test_full_workflow_with_failure_and_retry(self, mock_subprocess, temp_dir):
...
'''
        ...
        claude_success = Mock()
        claude_success.returncode = 0
        claude_success.stdout = "Claude executed"
        ...
        # Sequence: Claude success, verify fail, Claude success, verify fail, Claude success, verify success
        mock_subprocess.side_effect = [
            claude_success, verify_failure,  # Attempt 1
            claude_success, verify_failure,  # Attempt 2
            claude_success, verify_success,  # Attempt 3
        ]
    ...
    @patch('subprocess.run')
    def test_task_failure_stops_execution(self, mock_subprocess, temp_dir):
...
'''
        ...
        claude_success = Mock()
        claude_success.returncode = 0
        claude_success.stdout = "Claude OK"
        ...
        mock_subprocess.side_effect = [claude_success, verify_failure]
